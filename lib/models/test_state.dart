...
class TestState extends ChangeNotifier {
  ...
  List<Map<String, dynamic>> _getTestSequence() {
    return [
      ...
      // {'name': 'WiFi MAC读取', 'type': 'WiFi', 'executor': _autoTestWiFiMACRead, 'skippable': false},
      {'name': '14. 硬件版本号写入', 'type': '版本', 'executor': _autoTestWriteHardwareVersion, 'skippable': false},
      {'name': '15. 硬件版本号读取', 'type': '版本', 'executor': _autoTestReadHardwareVersion, 'skippable': false},
      
      // ========== 其他功能测试 ==========
      {'name': '16. EMMC容量检测测试', 'type': 'EMMC', 'executor': _autoTestEMMCCapacity, 'skippable': false},
      {'name': '17. 设备电压测试', 'type': '电压', 'executor': _autoTestVoltage, 'skippable': false},
      {'name': '18. 电量检测测试', 'type': '电量', 'executor': _autoTestBattery, 'skippable': false},
      {'name': '19. 充电状态测试', 'type': '充电', 'executor': _autoTestCharging, 'skippable': false},
      // {'name': '生成设备标识', 'type': '标识', 'executor': _autoTestGenerateDeviceId, 'skippable': false},
      // {'name': 'SPP蓝牙功能测试', 'type': '蓝牙', 'executor': _autoTestSppBluetooth, 'skippable': false},
      {'name': '20. WiFi功能测试', 'type': 'WiFi', 'executor': _autoTestWiFi, 'skippable': false},
      {'name': '21. Sensor测试', 'type': 'Sensor', 'executor': _autoTestSensor, 'skippable': false},
      {'name': '22. RTC设置时间测试', 'type': 'RTC', 'executor': _autoTestRTCSet, 'skippable': false},
  ...
  Future<void> _executeAllTests() async {
    ...
    for (var i = 0; i < testSequence.length; i++) {
      ...
      try {
        ...
        // WiFi、IMU、Touch、Sensor、蓝牙、MIC、SPK、LED、唤醒、指令测试内部已有完整的逻辑，不使用外层重试包装器
        // WiFi有重试，IMU/Touch/Sensor/蓝牙等待用户确认，MIC/SPK/LED有弹窗和完整流程
