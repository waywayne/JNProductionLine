...
# -*- coding: utf-8 -*-
"""
RFCOMM Bind 桥接脚本 - 原始数据透传版本
使用 rfcomm bind 创建 /dev/rfcomm0，然后桥接到 stdin/stdout
此版本不进行 GTP 组装，直接透传原始数据，由 Dart 端负责组装
"""
...
def log(message):
    """输出日志到 stderr"""
    print(f"[RFCOMM-BIND-RAW] {message}", file=sys.stderr, flush=True)
...
def device_to_stdout(device_fd, keep_alive_event):
    """从设备读取数据并直接输出到 stdout（原始透传，不进行 GTP 组装）"""
    log("🎧 开始监听设备数据（原始透传模式）...")
    recv_count = 0
    timeout_count = 0
    empty_read_count = 0
...
def main():
    if len(sys.argv) != 3:
        print("用法: rfcomm_bind_bridge_raw.py <MAC地址> <通道>", file=sys.stderr)
        sys.exit(1)
